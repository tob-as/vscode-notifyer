...
import { StateWatcher, ProjectState, ProjectsSummary } from "./stateWatcher.js";
...
function writeCommand(sharedDir: string, projectId: string, command: unknown): void {
...
function main(): void {
  ...
  watcher.on("state-change", (_projectId: string, state: ProjectState) => {
    broadcastSSE("state-change", state);
  });

  watcher.on("new-approval", (projectId: string, requestId: string) => {
    broadcastSSE("new-approval", { projectId, requestId });
  });

  watcher.on("project-removed", (projectId: string) => {
