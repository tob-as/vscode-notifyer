...
export interface ProjectState {
  ...
  /** v1.1.0: Unix timestamp (ms) of last text edit in any document, or null */
  lastTextEditAt: number | null;
  /** v1.1.0: Unix timestamp (ms) when this session started */
  sessionStartedAt: number;
}

/** Command types that can be sent from Stream Deck/Dashboard to VS Code */
export type CommandType =
  | "focus_approvals"
  | "open_notifications_center"
  | "respond_approval";

/** Base command sent via commands/<projectId>.ndjson */
...
/** Command to respond to an approval request (v1.2.0) */
export interface RespondApprovalCommand extends Command {
  type: "respond_approval";
  /** ID of the approval request to respond to */
  requestId: string;
  /** Response option (e.g., "Yes", "No") */
  response: string;
}
