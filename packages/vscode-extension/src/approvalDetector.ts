import { ApprovalDetectionResult, ApprovalSeverity } from "./types";

/**
 * Button patterns that indicate an approval prompt
 */
const APPROVAL_BUTTON_PATTERNS: Array<{
  buttons: string[];
 ...
 */
function matchesButtonPattern(
  options: string[]
): { matches: boolean; severity: ApprovalSeverity } {
  ...
  for (const pattern of APPROVAL_BUTTON_PATTERNS) {
    ...
    for (const button of patternSet) {
      if (!normalizedOptions.has(button)) {
        ...
        const hasPartialMatch = [...normalizedOptions].some(opt =>
          opt.includes(button) || button.includes(opt)
        );
        if (!hasPartialMatch) {
          allPresent = false;
          break;
  ...
  const hasDontAskAgain = [...normalizedOptions].some(
    opt => opt.includes("don't ask again") ||
 ...
 */
function containsApprovalKeywords(message: string): boolean {
 ...
 */
function containsExclusionKeywords(message: string): boolean {
  const lowerMessage = normalize(message);
 ...
 */
function detectSeverityFromMessage(message: string): ApprovalSeverity {
  const lowerMessage = normalize(message);
 ...
 */
export function extractCallerExtension(): string | null {
  try {
    ...
    const extensionMatch = stack.match(
      /\.vscode-server\/extensions\/([^/]+)-[\d.]+\//
    );
 ...
 */
export function isClaudeExtension(extensionId: string | null): boolean {
